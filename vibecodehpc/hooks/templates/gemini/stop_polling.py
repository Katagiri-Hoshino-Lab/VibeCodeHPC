#!/usr/bin/env python3
"""VibeCodeHPC Stop Hook for Polling Agents (Gemini CLI).

Prevents agent idle by blocking Stop events (exit code 2) and
re-injecting context with probabilistic file embedding.

Mirrors the Claude Code stop_polling.py hook, adapted for Gemini's
hooks.json protocol.

Gemini stdin: {"event": "Stop", ...}
Exit code 2 → block the Stop event.
Stderr output → re-injected as context.
"""
...
def get_stop_count() -> int:
    ...
    path = Path.cwd() / ".gemini" / "hooks" / "stop_count.txt"
    if path.exists():
        try:
...
def increment_stop_count() -> int:
    ...
    hooks_dir = Path.cwd() / ".gemini" / "hooks"
...
_ROLE_TASKS = {
    "PM": ["Check progress of all agents", "Budget management", "Intervene with stalled agents"],
    "SE": ["Monitor ChangeLog.md updates", "Generate SOTA history graphs", "Check job results"],
    "PG": ["Update ChangeLog.md and manage SOTA", "Check job queue", "Implement optimizations"],
    "CD": ["Check SOTA-achieving code", "Anonymization and git push", "Verify .gitignore"],
    "SOLO": ["Check ChangeLog.md", "Implement optimizations", "Check job results", "SOTA determination", "Check budget"],
}
...
2. Update ToDo list
3. Select and execute priority tasks
4. Report progress via agent_send.sh
5. Repeat 1-5

[Communication] agent_send.sh [recipient] "[{agent_id}] message"
